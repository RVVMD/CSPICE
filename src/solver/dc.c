            ...
            for (int i = 0; i < solver->num_components; i++) {
                ...
                if (comp->type == MNA_CUSTOM_NONLINEAR) {
                    ...
                    for (int j = 0; j < npole->num_nodes; j++) {
                        ...
                        npole->last_values[j] = damping * current_val + 
